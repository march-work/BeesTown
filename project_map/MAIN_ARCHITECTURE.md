...
BeesTown 是一个**面向多 Agent 集群协作的 CLI 编辑器类工具**：用于从零创建一个项目，或在项目中途进入并持续维护。用户通过自然语言或手动编排的方式，组合 Agent 与工具，完成代码开发、文件操作、项目管理等协作任务的自动化执行。

其中“公司/部门/员工（Agent）”是为了描述组织关系、资源继承与审批链路的**交互隐喻**，并不意味着一定存在真实的多用户公司系统。

当前产品边界默认设定为：**仅支持单人类用户（单操作者）**。所有审批链路最终指向同一个人类用户。

### 核心理念
- **公司隐喻**：每个项目 = 一家独立公司（用于组织与权限建模）
- **双通道路由**：确定性工具命令直达执行引擎，自然语言（NL）请求经由 HR 协调
- **HR 为入口（NL）**：HR Agent 作为自然语言入口，负责招聘、人员管理与跨角色协作分发
- **层级化管理**：清晰的上下级汇报关系和信息流
- **双重记忆**：部门/层级共享记忆 + 个人独立记忆
- **资源分层**：工具、MCP、技能可在部门/个人层面配置
- **分级审批**：下级工具调用需上级审批，上级再按规则向人类用户申请批准
...
### 3.1 项目（被管理工作区）= 公司隐喻模型

每个 BeesTown 项目是一个被 CLI 管理的“工作区实体”，可用于创建新项目或进入既有项目进行维护；“公司”只是用于表达组织关系与协作机制的隐喻。
...
公司级共享资源用于表达“全公司默认可用”的资源（例如公司级默认工具集、默认 MCP、公司级记忆/规则）。为减少两套容器并存的复杂度，推荐将其实现为一个特殊的 `level=0` 部门：

- 公司级资源 = `level=0` 部门的 `shared`
- 业务部门（如技术部/产品部/运营部）在其 `shared` 之上叠加与覆盖

这样资源继承链路保持一致：公司级（level=0）→ 部门 → 个人。
...
1. **下级 Agent 调用工具需要其直接上级批准**
2. **LV1（HR/项目管理）调用工具需要 LV0（人类用户）批准**
3. **LV2+（开发/测试等）无需直接请求人类批准**，但必须经由上级 Agent 审批后方可执行
